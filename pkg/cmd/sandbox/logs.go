...
func newSandboxLogs() *cobra.Command {
	cmd := &cobra.Command{
		...
		// Hidden: the agent modal log channel is the only stream this
		// surfaces, and it's empty for sandboxes with no agent_type set
		// (which is every dev sandbox today). Use the sandbox shell or
		// Axiom for stdout/stderr until the stream is broadened.
		Hidden: true,
