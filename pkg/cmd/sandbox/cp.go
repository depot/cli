...
// cpRemoteToLocal streams remote `tar c` → local `tar x` over the binary-safe
// stdout_raw rail introduced for DEP-4404. No base64 detour needed.
func cpRemoteToLocal(ctx context.Context, token, orgID, sandboxID, sessionID, src, dst string, stderr io.Writer) error {
	...
	stream, err := client.RemoteExec(ctx, api.WithAuthenticationAndOrg(connect.NewRequest(&civ1.ExecuteCommandRequest{
		...
		SessionId: sessionID,
		Command: &civ1.Command{
			CommandArray: []string{"/bin/sh", "-c", `exec tar c -C "$1" -f - "$2"`, "depot-cp", srcDir, srcBase},
		},
	}), token, orgID))
	if err != nil {
		_ = tarStdin.Close()
	...
	for stream.Receive() {
		msg := stream.Msg()
		// Prefer the binary-safe raw rail. Tar streams are non-UTF-8, so the
		// legacy rail would be empty for the actual archive bytes.
		if len(msg.StdoutRaw) > 0 {
			if _, werr := tarStdin.Write(msg.StdoutRaw); werr != nil {
				_ = tarStdin.Close()
				_ = tarCmd.Process.Kill()
				_ = tarCmd.Wait()
		...
		case *civ1.ExecuteCommandResponse_Stderr:
			// Legacy rail fallback (older api server with no raw stderr).
			// Skip when the raw rail already carried these bytes.
			if len(msg.StderrRaw) == 0 {
		...
		case *civ1.ExecuteCommandResponse_ExitCode:
			...
			waitErr := tarCmd.Wait()
			if v.ExitCode != 0 {
				return fmt.Errorf("remote tar c exited %d", v.ExitCode)
			}
			if waitErr != nil {
