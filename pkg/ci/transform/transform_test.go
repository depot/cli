...
// Local .github/ references are resolved by Depot wherever they lead, so migrate
// leaves every form of them exactly as written — action refs, reusable workflow
// calls, sparse-checkout inputs, script paths, and paths inside comments.
func TestTransformWorkflow_PreservesGitHubPaths(t *testing.T) {
	raw := []byte(`name: CI
# Helpers live in .github/actions/
on: push
jobs:
  build:
    runs-on: depot-ubuntu-latest
    steps:
      ...
      - run: .github/workflows/scripts/build.sh # see .github/actions/deploy
...
`)
	...
	content := string(result.Content)
	if strings.Contains(content, ".depot/") {
		t.Errorf("expected no .depot/ rewrites, got:\n%s", content)
	}
