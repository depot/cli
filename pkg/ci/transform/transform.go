...
//
// Local `.github/` references — `uses: ./.github/actions/foo`, reusable workflow
// calls, script paths in `run:` blocks — are deliberately left as written. Depot
// resolves them wherever they lead, so rewriting them to `.depot/` would only add
// churn to the migrated file and break references to anything that stayed behind.
