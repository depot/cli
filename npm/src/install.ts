...
import {DEPOT_BINARY_PATH, downloadedBinPath, isValidBinaryPath, pkgAndSubpathForCurrentPlatform} from './platform'

const versionFromPackageJSON: string = require(path.join(__dirname, 'package.json')).version.split('-cli.')[1]
const toPath = path.join(__dirname, 'bin', 'depot')
let isToPathJS = true
...
function validateBinaryVersion(...command: string[]): void {
  ...
  try {
    ...
    if (os.platform() === 'darwin' && /_SecTrustEvaluateWithError/.test(err + '')) {
      ...
      throw new Error(`The "@depot/cli" package cannot be installed because ${os} is too outdated.

The Go compiler (which depot relies on) no longer supports ${os},
...
`)
  ...
  const versionFromStdout = matches?.[1]
...
function installUsingNPM(pkg: string, subpath: string, binPath: string): void {
  // Erase "npm_config_global" so that "npm install --global @depot/cli" works.
  ...
  // Create a temporary directory inside the "@depot/cli" package with an empty
...
function applyManualBinaryPathOverride(overridePath: string): void {
  // Patch the CLI use case (the "depot" command)
  ...
  // Patch the JS API use case (the "require('@depot/cli')" workflow)
...
function maybeOptimizePackage(binPath: string): void {
  // This package contains a "bin/depot" JavaScript file that finds and runs
  // the appropriate binary executable. However, this means that running the
  // "depot" command runs another instance of "node" which is way slower than
  ...
  // because on Windows the binary executable must be called "depot.exe"
  // instead of "depot".
  //
  ...
  if (os.platform() !== 'win32' && !isYarn()) {
    const tempPath = path.join(__dirname, 'bin-depot')
    try {
...
`)
    ...
    // to be helpful but it's not the supported way of installing @depot/cli.
